...
import { exportAsHtml } from "@/lib/export-contact-sheet";
...
export interface GalleryFrame {
    ...
    isFirst: boolean; // first frame gets premium blur entrance
}
...
export function GalleryPanel({
    ...
    const getExportFrames = useCallback(() => {
        return isSelecting
            ? frames.filter((f) => selectedIds.has(f.id))
            : frames;
    }, [frames, selectedIds, isSelecting]);

    const handleExportHtml = useCallback(() => {
        exportAsHtml(getExportFrames(), videoName);
    }, [getExportFrames, videoName]);
                        ...
                        {frames.length > 0 && (
                            <>
                                ...
                                <Tooltip>
                                    ...
                                    <TooltipContent side="top">Export HTML contact sheet</TooltipContent>
                                </Tooltip>
                            </>
                        )}
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="absolute bottom-1.5 left-1.5 pointer-events-none">
                                        <span className="inline-flex items-center px-1.5 py-0.5 rounded-md bg-black/60 backdrop-blur-sm font-mono text-[9px] text-zinc-200 tabular-nums tracking-wider ring-1 ring-white/[0.08]">
                                    ...
                                    <button
                                        ...
                                        className="absolute top-1 right-1 flex items-center justify-center w-5 h-5 rounded-full bg-black/60 hover:bg-red-500/60 text-white/70 hover:text-white opacity-0 group-hover:opacity-100 transition-all duration-200 hover:scale-110 z-10"
                                    >
                                        <X className="w-3 h-3" />
                                    </button>

                                    {/* Hover overlay with download — only when expanded */}
                                    {isExpanded && (
                                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/40 transition-opacity duration-200">
                                            <button
                                                ...
                                                className="flex items-center justify-center w-7 h-7 rounded-full bg-black/50 hover:bg-white/30 text-white transition-all duration-200 hover:scale-110"
                                            >
                                                <Download className="w-3 h-3" />
                                            </button>
                                        </div>
